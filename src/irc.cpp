...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #swagcoinTEST3\r");
            Send(hSocket, "WHO #swagcoinTEST3\r");
        } else {
            // randomly join #swagcoin00-#swagcoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #swagcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #swagcoin%02d\r", channel_number).c_str());
        }
