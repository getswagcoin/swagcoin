...
// Copyright (c) 2013-2014 Swagcoin Developers
...
uint256 hashGenesisBlock("0xb81cbe2f52be325939f6c6960288ae36df62cc416c05a9da00a7fb97dbea5081");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Swagcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Swagcoin Signed Message:\n";
...
{
    // Swagcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    int64 nSubsidy = 450 * COIN;
 
    nSubsidy >>= (nHeight / 258008); // Subsidy is halved every 58008 blocks
     
    if(nHeight == 1)
    {
        nSubsidy = 5318008 * COIN; // PREMINE
    }
    else if(nHeight < 60) // PREMINE CURING
    {
        nSubsidy = 0 * COIN; 
    }
    else if(nHeight < 900) // EASYIN AKA ANTI-INSTAMINE
    {
        nSubsidy = 1 * COIN;
    }
    ...
    {
        nSubsidy = 9001 * COIN; // BLING BLING BLOCK - IT'S OVER 9000
    }
    else if(rand >= 300 && rand < 2800)
    {
        nSubsidy = 2250 * COIN; // BLING BLOCK - IT'S OVER 9000
    }

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // SwagCoin: every 1440 blocks
static const int64 nTargetSpacing = 60 ; // SwagCoin: gone in 60 seconds
...
unsigned int static GetNextWorkRequired_v1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Swagcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                ...
                if (pindexLast->nHeight+1 >= 11000) { DiffMode = 2; }
        }
...
{
    // Swagcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SwagcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SwagcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SwagcoinMiner(CWallet *pwallet)
{
    printf("SwagcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("swagcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SwagcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SwagcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SwagcoinMiner, pwallet));
}
